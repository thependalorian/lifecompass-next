...
async function testHuggingFaceEmbedding() {
  const apiKey =
    process.env.HUGGINGFACE_API_KEY || process.env.EMBEDDING_API_KEY;
  ...
  if (!apiKey) {
    console.error(
      "❌ Error: HUGGINGFACE_API_KEY or EMBEDDING_API_KEY not found in environment",
    );
    process.exit(1);
  }
  ...
  const testText =
    "LifeCompass is a comprehensive CRM and AI assistant for insurance advisors.";
