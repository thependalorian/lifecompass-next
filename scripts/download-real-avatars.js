...
async function downloadAllAvatars() {
  console.log(
    "Downloading 20 real professional advisor headshots from Unsplash...\n",
  );
  console.log(
    "Note: These are real professional portraits from free stock photo services\n",
  );

  const promises = [];
  for (let i = 0; i < 20; i++) {
    promises.push(
      downloadFromUnsplash(i).catch((err) => {
        console.error(
          `✗ Error downloading advisor-${String(i + 1).padStart(2, "0")}:`,
          err.message,
        );
        return null;
      }),
