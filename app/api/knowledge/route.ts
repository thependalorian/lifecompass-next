...
const CONSOLIDATED_DIR = path.join(process.cwd(), '..', 'LifeCompass', 'old_mutual_complete_crawl', 'consolidated');
  ...
  try {
    ...
    if (!category && !query) {
      try {
        ...
        const consolidatedFiles = files.filter(file => file.endsWith('_CONSOLIDATED.md'));

        return NextResponse.json({
          available_categories: consolidatedFiles.map(file => file.replace('_CONSOLIDATED.md', '')),
    ...
    if (category) {
      try {
        ...
        const overview = lines.find(line => line.startsWith('**Consolidated from')) || '';
        ...
        results: [
          {
            ...
            snippet: "Coverage for 68 severe illnesses with lump sum payment upon diagnosis...",
            relevance_score: 0.95
          },
          {
            category: "INVESTMENT_RETIREMENT",
            title: "Unit Trust Information",
            snippet: "Professional investment portfolios with diversification across asset classes...",
            relevance_score: 0.89
          },
          {
            category: "DEATH_CLAIMS",
            title: "Claims Process",
            snippet: "Final expenses benefits paid within 48 hours, other claims within 15 working days...",
...
export async function POST(request: NextRequest) {
  try {
        ...
        INSURANCE_PRODUCTS: [
          {
            ...
            features: ["Lump sum payments", "Family protection benefits", "Cashback options"],
          ...
          },
          {
            ...
            features: ["48-hour claims payment", "Premium holidays", "Cashback benefits"],
    ...
    if (action === "get_claims_info") {
      ...
      return NextResponse.json({
        death_claims: {
          payment_times: "Final expenses: 48 hours, Other claims: 15 working days",
          requirements: ["Death certificate", "Beneficiary form", "Banking details"],
          contact: "061 223 189"
        },
        disability_claims: {
