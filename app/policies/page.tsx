...
export default function PoliciesPage() {
    ...
    const fetchPolicies = async () => {
      ...
      try {
        const response = await fetch(
          `/api/policies?customerNumber=${selectedPersona}`,
        );
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      ...
      try {
        const response = await fetch(
          `/api/activity?customerNumber=${selectedPersona}&limit=10`,
        );
        if (response.ok) {
          const data = await response.json();
          setActivities(data);
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div
                          className={`badge ${getStatusBadge(policy.status)}`}
                        >
                          {policy.status}
                        </div>
                      </div>
                    ...
                    <div className="flex gap-2 mt-4 lg:mt-0 lg:flex-col lg:gap-2">
                      <Link
                        href={`/policies?policy=${encodeURIComponent(policy.policyNumber || policy.id)}`}
                        className="btn-om-primary btn-sm"
                      >
                        View Details
                      </Link>
                      <Link href="/products" className="btn-om-outline btn-sm">
                        Download
                      </Link>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="card-om p-6">
            {loadingActivities ? (
              ...
              <div className="text-center py-8 text-om-grey">
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-om-grey/70 mt-1">
                          {activity.details}
                        </p>
                      </div>
                    </div>
